...
const Increment= ()=> {
  const dispatch = useDispatch()
    ...
    const handleCheckAllChange = (e) => {
      setChecked(
        e.target.checked ? employee.map((c) => c.id) : []
      );
    };
    ...
    return (
      <div className="container" style={{height:'400px',width:'300px',border:'1px solid black'}}> 
        ...
        {employee.map((c) => (
          <div className="form-check form-check-inline" style={{display:'flex',marginLeft:'15px'}} key={c.id}>
